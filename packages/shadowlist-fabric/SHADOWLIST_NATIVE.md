...
| `setData(items, { scrollTo? })`                                          | Same as a new `data` array. `scrollTo: 'start'` also goes to offset 0 in the commit that reconciles the new rows (see "Scroll commands").                                                           |
...
| `scrollToStart()`                                                        | Offset 0 with the header in view, sequenced like `scrollToEnd` (after every earlier mutation is laid out). To show replaced rows from the top, use `setData(items, { scrollTo: 'start' })` instead. |
...
about 18 with a finger held. Offset 0 does not depend on the new rows' sizes, which is why this
scroll can skip the wait.
