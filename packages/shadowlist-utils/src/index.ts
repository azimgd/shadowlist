...
/*
 * Activity demo: shared data shape, generator and the interactive-control config
 * (threshold presets, scroll distance after which the sticky header hides).
 */
...
export function buildActivity(index: number): ActivityData {
  const actor = CHARACTER_NAMES[index % CHARACTER_NAMES.length]!;
  return {
    id: generateUniqueId(),
    actor,
    action: ACTIVITY_ACTIONS[index % ACTIVITY_ACTIONS.length]!,
    detail: generateRandomText(index).slice(0, 90),
    timestamp: `${(index % 59) + 1}m`,
